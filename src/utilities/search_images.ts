...
export const is_image_there = (image_name:string):Promise<string|undefined>=>{
  const image_path  = get_main_path(image_name);
  return is_there(image_path);
};
export const is_image_cashed = (image_name:string, width:number, height:number ):Promise<string|undefined>=>{
