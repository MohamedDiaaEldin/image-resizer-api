...
describe('test build path function', ()=>{
  ...
  const expected_cashed_path =path.join(abslout_path, 'images', 'cashed', `${image_name}_${width}_${hegiht}.webp`)
  const expected_main_path = path.join(abslout_path,'images', 'main', `${image_name}.jpg`);
